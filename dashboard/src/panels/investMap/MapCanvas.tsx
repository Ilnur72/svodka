 ...
 *
 * Тарих: 74 (7 маркер учун) → 46 (55 маркерда экран тўлиб кетарди) → 62.
 * Охиргиси ситуацион марказ экрани учун: 10 метрли мониторда 46 px кристалл
 * узоқдан кўринмай қоларди. 62 px — узоқдан танилади, лекин ўндан ортиқ
 * маркер ёнма-ён турганда ҳам фон кўринади.
 */
const MARKER_H = 62;
...
/**
 * Ёрлиқнинг КРИСТАЛЛ четидан оралиғи, px. CSS билан бир хил бўлиши ШАРТ.
 *
 * ⚠️ Кристалл четидан, PNG четидан ЭМАС. Расмнинг катта қисми шаффоф
 * (кристалл марказда, атрофида ёғду), шунинг учун тўлиқ энидан ҳисобланса
 * ёрлиқ кўзга кристаллдан узилиб қолгандай кўринарди — фойдаланувчи буни
 * «маркер ва номи ораси очилиб қолган» деб айтди. Энди оралиқ шаффоф
 * ҳошиядан кейин бошланади: қаранг `ICON_HIT_W` ва CSS даги `81%`.
 */
const LABEL_GAP = 2;
 ...
 * 192 объект учун бундай жадвални ёзиб бўлмайди.
 */
 ...
 *
 * ⚠️ `invest` → `#registry` («Лойиҳалар реестри»), `#invest` ЭМАС: хаританинг
 * бу қатлами энди `project_registry_projects` (144 лойиҳа) дан келади, ўша
 * лойиҳалар айнан «Лойиҳалар реестри» бўлимида тўлиқ кўрсатилади. `#invest`
 * («Инвестиция лойиҳалари») бошқа манба (`invest_projects`, 7 лойиҳа).
 */
...
export function MapCanvas({
  ...
  /**
   * ═══ 192 ёрлиқ экранни босиб кетмаслиги учун ════════════════════════════
   *
   ...
   *     7 нуқта учун headless браузерда ўлчаб ёзилган эди. 192 объект учун
   *     уни ёзиб бўлмайди, ёзилса ҳам базага битта янги лойиҳа қўшилиши
   *     билан эскирарди.
   *
   *   · **Доим кўринадиган 192 ёрлиқ — ўқиб бўлмайдиган экран.** Бошланғич
  ...
  const layoutLabels = useCallback(() => {
    ...
    for (const c of cands) {
      ...
      const boxAt = (right: boolean, dy: number): Box => ({
        x:
          (right
            ? c.cx + (markerW * ICON_HIT_W) / 2 + LABEL_GAP
            : c.cx - (markerW * ICON_HIT_W) / 2 - LABEL_GAP - c.w) -
          LABEL_PAD,
