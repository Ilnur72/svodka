 ...
 *
 * ═══ Файл номи БАЗАДАН келади, `id` дан ясалмайди ═══════════════════════
 *
 * Манзил `image` устунидан қурилади (`"094.jpg"` → `registry/094.jpg`).
 * Бэкенд ФАҚАТ файл номини беради — йўл ҳам, хост ҳам йўқ, тўлиқ манзилни
 * истеъмолчи ўзи йиғади (`geology_projects.photoPath` билан бир хил
 * контракт). Аввал бу ном `id` дан ясаларди (`registry/<id>.jpg`), лекин
 * файллар тақдимотдаги ўз тартиб рақами билан аталган ва иккови ҳамма
 * жойда мос келмайди — шунинг учун ягона манба база бўлди.
 *
 * ⚠️ Сурат лойиҳа НОМИГА ҚАРАБ танланмайди ва бу ерда ҳеч нарса «тахмин»
 * қилинмайди: реестрда 144 лойиҳа бор, уларнинг номи «Инвестиция
 * лойиҳалари» бўлимидаги еттита сурат номи билан ўхшаш бўлиши мумкин —
 * ўхшашликка қараб бириктирилган сурат бутунлай бошқа объектни кўрсатиб
 * қўярди ва буни экрандан сезиб бўлмасди (координата билан бир хил ҳолат,
 * қаранг: `project-registry.regions.ts`). Боғланиш фақат `image` бўйича.
 *
 * ═══ «Сурат йўқ» ≠ «сурат юкланмади» ════════════════════════════════════
 *
 * `image === null` (144 дан 100 таси) — манбада сурат бириктирилмаган, бу
 * МАЪЛУМ ҳолат. `image` бор-у файл келмаса — бу НОСОЗЛИК. Шунинг учун
 * `src` иккинчи ҳолатда ҳам берилади, лекин иккита изоҳ матни алоҳида
 * тайёрланади ва `AreaPhoto` уларни ҳар хил сарлавҳа билан кўрсатади.
 * Ҳар қандай ҳолатда остидаги кўрсаткичлар чизилаверади — улар суратга
 * умуман боғлиқ эмас.
 */
export interface RegArea {
  /**
   * `registry/<файл номи>` манзили, ёки `null` — реестрда бу лойиҳа учун
   * сурат бириктирилмаган (`image === null`).
   */
  src: string | null;
  alt: string;
  ...
  note: string;
  /** Плашканинг иккинчи қатори — расм ЮКЛАНМАГАНДА (манзил бор эди). */
  photoNote: string;
  /** Плашканинг иккинчи қатори — сурат УМУМАН бириктирилмаганда. */
  photoEmptyNote: string;
}
...
export function regDetail(p: RegProject, all: RegProject[], emptyColumns: string[]): RegDetail {
  ...
  // Бўш сатр «сурат бор, номи бўш» деган ёлғон ҳолат бўларди — `txt()` уни
  // ҳам `null` га келтиради, яъни экранда «сурат йўқ» бўлиб кўринади.
  const image = txt(r.image);
  const imageFilled = filledCount(all, (x) => isSet(x.image));
  const area: RegArea = {
    // `import.meta.env.BASE_URL` — илова илдиз каталогда турмаса ҳам манзил
    // тўғри бўлиши учун (`vite.config.ts` да `base: "./"`).
    src: image === null ? null : `${import.meta.env.BASE_URL}registry/${image}`,
    ...
    // Носозлик матни: базада ном БОР, демак файл `public/registry/` да
    // бўлиши керак эди. Номи очиқ ёзилади — акс ҳолда қайси файл йўқолганини
    // экрандан билиб бўлмасди.
    photoNote:
      image === null
        ? ""
        : `Базада «${image}» кўрсатилган, лекин файл юкланмади — у «public/registry/» да йўқ ёки деплойда тушиб қолган.`,
    photoEmptyNote: `Реестрда сурат ${nf(imageFilled)} / ${nf(total)} лойиҳага бириктирилган, бу лойиҳа уларнинг орасида эмас. Қуйидаги кўрсаткичлар реестрдан олинган ва суратга боғлиқ эмас.`,
  };
