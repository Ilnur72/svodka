 ...
 *  - фоиз **қайта ҳисобланмайди** — бэкенддан келгани сақланади. Диаграмма
 *    учун алоҳида `barPct` чегараланган қиймат берилади (`PCT_MAX`), ҳақиқий
 *    сон эса матнда тўлиқ қолади.
 */

/** Фоиз ўқининг юқори чегараси: ундан катта қиймат устунни бузмайди. */
export const PCT_MAX = 150;

/** Ўқдаги «100%» белгисининг ўрни (устун кенглигининг фоизи). */
export const PCT_REF_AT = (100 / PCT_MAX) * 100;
...
export interface BalanceCellVM {
  ...
  pct: number | null;
  /** Устун учун `0…PCT_MAX` оралиғига келтирилган фоиз. */
  barPct: number | null;
...
function toCell(v: BalanceValue | null | undefined): BalanceCellVM | null {
  ...
  return {
    ...
    fakt,
    pct,
    barPct: pct === null ? null : Math.max(0, Math.min(pct, PCT_MAX)),
...
/** Гуруҳдаги барча тармоқлар — якуний тугун охирида. Жадвал (Б) блоки учун. */
export const groupBranches = (g: BalanceGroupVM): BalanceBranchVM[] =>
  g.merge ? [...g.branches, g.merge] : g.branches;
